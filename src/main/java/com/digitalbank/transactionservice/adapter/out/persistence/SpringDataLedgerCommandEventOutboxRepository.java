...
interface SpringDataLedgerCommandEventOutboxRepository
        extends JpaRepository<LedgerCommandEventOutboxJpaEntity, UUID> {

    List<LedgerCommandEventOutboxJpaEntity> findTop100ByEventStatusInAndAvailableAtLessThanEqualOrderByCreatedAtAsc(
            List<LedgerCommandOutboxStatus> statuses, Instant now);

    List<LedgerCommandEventOutboxJpaEntity> findByProcessingTokenOrderByCreatedAtAsc(UUID processingToken);

    @Modifying(flushAutomatically = true)
    @Query(value = """
            with claim_lock as materialized (
                select pg_try_advisory_xact_lock(
                        hashtextextended('transaction-service.ledger-command-event-outbox', 0)) as acquired
            ), candidates as (
                select outbox.event_id
                from ledger_command_event_outbox outbox
            ...
            update ledger_command_event_outbox outbox
    ...
    @Modifying(flushAutomatically = true)
    @Query("""
            update LedgerCommandEventOutboxJpaEntity outbox
    ...
    int claimReadyH2(
            ...
            @Param("statuses") List<LedgerCommandOutboxStatus> statuses,
            @Param("now") Instant now,
            @Param("claimToken") UUID claimToken,
            @Param("leaseUntil") Instant leaseUntil);
    ...
    @Modifying(flushAutomatically = true)
    @Query(value = """
            update ledger_command_event_outbox
    ...
    @Query("""
            update LedgerCommandEventOutboxJpaEntity outbox
               set outbox.eventStatus = com.digitalbank.transactionservice.adapter.out.persistence.LedgerCommandOutboxStatus.PUBLISHED,
    ...
    @Query("""
            update LedgerCommandEventOutboxJpaEntity outbox
               set outbox.eventStatus = com.digitalbank.transactionservice.adapter.out.persistence.LedgerCommandOutboxStatus.FAILED,
    ...
    @Modifying(flushAutomatically = true)
    @Query(value = """
            insert into ledger_command_event_outbox (
                event_id, event_type, schema_version, producer, occurred_at,
                aggregate_id, correlation_id, causation_id, transaction_id,
                reservation_request_id, reservation_id, posting_request_id, source_account_id,
                destination_account_id, amount, currency, event_status, attempt_count,
                available_at, json_payload, created_at
            ) values (
                :eventId, :eventType, :schemaVersion, :producer, :occurredAt,
                :aggregateId, :correlationId, :causationId, :transactionId,
                :reservationRequestId, :reservationId, :postingRequestId, :sourceAccountId,
                :destinationAccountId, :amount, :currency, 'PENDING', 0,
    ...
    @Modifying(flushAutomatically = true)
    @Query(value = """
            merge into ledger_command_event_outbox (
                event_id, event_type, schema_version, producer, occurred_at,
                aggregate_id, correlation_id, causation_id, transaction_id,
                reservation_request_id, reservation_id, posting_request_id, source_account_id,
                destination_account_id, amount, currency, event_status, attempt_count,
            ...
            select :eventId, :eventType, :schemaVersion, :producer, :occurredAt,
                   ...
                   :availableAt, :jsonPayload, :createdAt
            where not exists (
                select 1 from ledger_command_event_outbox

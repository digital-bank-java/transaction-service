...
interface SpringDataTransferTerminalEventOutboxRepository
        extends JpaRepository<TransferTerminalEventOutboxJpaEntity, UUID> {

    List<TransferTerminalEventOutboxJpaEntity> findTop100ByEventStatusInAndAvailableAtLessThanEqualOrderByCreatedAtAsc(
            List<TransferTerminalOutboxStatus> statuses, Instant now);

    List<TransferTerminalEventOutboxJpaEntity> findByProcessingTokenOrderByCreatedAtAsc(UUID processingToken);

    @Modifying(flushAutomatically = true)
    @Query(value = """
            with claim_lock as materialized (
                select pg_try_advisory_xact_lock(
                        hashtextextended('transaction-service.transfer-terminal-event-outbox', 0)) as acquired
            ), candidates as (
                select outbox.event_id
                from transfer_terminal_event_outbox outbox
            ...
            update transfer_terminal_event_outbox outbox
    ...
    @Modifying(flushAutomatically = true)
    @Query("""
            update TransferTerminalEventOutboxJpaEntity outbox
    ...
    int claimReadyH2(
            ...
            @Param("statuses") List<TransferTerminalOutboxStatus> statuses,
            @Param("now") Instant now,
            @Param("claimToken") UUID claimToken,
            @Param("leaseUntil") Instant leaseUntil);
    ...
    @Modifying(flushAutomatically = true)
    @Query(value = """
            update transfer_terminal_event_outbox
    ...
    @Query("""
            update TransferTerminalEventOutboxJpaEntity outbox
               set outbox.eventStatus = com.digitalbank.transactionservice.adapter.out.persistence.TransferTerminalOutboxStatus.PUBLISHED,
    ...
    @Query("""
            update TransferTerminalEventOutboxJpaEntity outbox
               set outbox.eventStatus = com.digitalbank.transactionservice.adapter.out.persistence.TransferTerminalOutboxStatus.FAILED,
    ...
    @Modifying(flushAutomatically = true)
    @Query(value = """
            insert into transfer_terminal_event_outbox (
                ...
                transfer_request_id, reservation_request_id, reservation_id,
                posting_request_id, posting_id, status, failure_stage, failure_code,
                failure_reason, compensation_status, manual_review_required,
                event_status, attempt_count, available_at, json_payload, created_at
            ) values (
                ...
                :transferRequestId, :reservationRequestId, :reservationId,
                :postingRequestId, :postingId, :status, :failureStage, :failureCode,
                :failureReason, :compensationStatus, :manualReviewRequired,
                'PENDING', 0, :availableAt, :jsonPayload, :createdAt
    ...
    int recordIfAbsent(
            ...
            @Param("manualReviewRequired") Boolean manualReviewRequired,
    ...
    @Modifying(flushAutomatically = true)
    @Query(value = """
            merge into transfer_terminal_event_outbox (
                event_id, event_type, schema_version, producer, occurred_at,
                aggregate_id, correlation_id, causation_id, transaction_id,
                source_account_id, destination_account_id, amount, currency,
                transfer_request_id, reservation_request_id, reservation_id,
            ...
            select :eventId, :eventType, :schemaVersion, :producer, :occurredAt,
                   ...
                   'PENDING', 0, :availableAt, :jsonPayload, :createdAt
            where not exists (
                select 1 from transfer_terminal_event_outbox where aggregate_id = :aggregateId
            )
            """, nativeQuery = true)
